...
void insert(token tk, char *arg){
    for(int i=0;i<ind;i++){
        if(strcmp(tk.name, tb[i].name)==0) return;
    }
...
token getNextToken(FILE *fp){
    ...
    while(1){
        ...
        if(c == '#'){ skipPP(fp); continue;}
        if(c== '/'){skipComments(fp); continue;}
        tk.row=row;
        tk.col=col;
        ...
        if(isalnum(c) || c == '$'){
            ...
            col+=strlen(buf);
            ungetc(c, fp);
            if(buf[0]=='$' || isKeyword(buf)) strcpy(tk.type, "IDENTIFIER");
            else strcpy(tk.type, "FUNCTION");
            strcpy(tk.name, buf);
            return tk;
        }
        ...
        if(c == '<' || c == '>' || c=='=' || c=='&'|| c == '|' || c == '!'){
            ...
            if((c == '&' && d == '&') || (c == '|' && d == '|')){
                ...
                if(c=='!')
                strcpy(tk.type, "LOGICAL OPERATOR");
                else strcpy(tk.type, "Relational OPERATOR");
            }
            strcpy(tk.name, buf);
            return tk;
        }
...
int main(){
    ...
    char c = fgetc(fp);
    while(c!=EOF){
        if(c== '/'){skipComments(fp);c = fgetc(fp);continue;}
        if(c == '<'){
            char d = fgetc(fp);
            if(d == '?'){
            while(c != EOF && c != '\n') c=fgetc(fp);
    ...
    while(1){
        ...
        if(strcmp(tk.type, "FUNCTION")==0){
            ...
            else if(strcmp(next.type, "STRING") == 0) strcpy(buf, next.name);
            insert(tk, buf);
        }
    }
    rewind(out);
